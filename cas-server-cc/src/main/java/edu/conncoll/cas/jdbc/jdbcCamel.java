		...
		switch (Interrupts.toInt(flag)) {
			...
			case PECIC:	
				...
				try {
					...
					//Parents
					SQL="select PARENT_PPID, PARENT_ORDER, PARENT_LEGAL_PREFIX_NAME, PARENT_PREF_FIRST_NAME, PARENT_PREF_MIDDLE_NAME, PARENT_PREF_LAST_NAME, PARENT_LEGAL_SUFFIX_NAME from cc_adv_peci_parents_t where CHANGE_COLS !='DELETE' and STUDENT_PIDM=:STUDENT_PIDM order by PARENT_ORDER";
					parentData = jdbcCAS.queryForList(SQL,namedParameters);
					
					//Emergency Contacts
					SQL="select PARENT_PPID, EMERG_CONTACT_PRIORITY, EMERG_LEGAL_PREFIX_NAME, EMERG_PREF_FIRST_NAME,EMERG_PREF_MIDDLE_NAME,EMERG_PREF_LAST_NAME, EMERG_LEGAL_SUFFIX_NAME from cc_gen_peci_emergs_t where CHANGE_COLS !='DELETE' and STUDENT_PIDM=:STUDENT_PIDM order by EMERG_CONTACT_PRIORITY";
