...
CREATE TABLE `cc_adv_peci_parents_t` (
  ...
  `DUPLICATE_STATUS` varchar(20) DEFAULT NULL,
...
CREATE TABLE `cc_gen_peci_addr_data_t` (
  ...
  `ADDR_SEQUENCE_NO` varchar(25) DEFAULT NULL,
...
ALTER TABLE `cas`.`peci_trans_start` 
	ADD COLUMN `STUDENT_UUID` VARCHAR(128) NULL AFTER `STUDENT_PIDM`;
	
ALTER TABLE `cas`.`cc_gen_peci_phone_data_t` 
	ADD COLUMN `ADDR_TYPE_CODE` VARCHAR(5) NULL AFTER `EMERG_NO_CELL_PHONE`,
	ADD COLUMN `ADDR_SEQNO` INT NULL AFTER `ADDR_TYPE_CODE`;
