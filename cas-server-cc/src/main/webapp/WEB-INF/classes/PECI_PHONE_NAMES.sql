...
BEGIN
	...
	CREATE temporary TABLE IF NOT EXISTS `PECI_Phones` AS 
		(SELECT distinct cast(null as CHAR(1)) PECI_PHONE_CODE, cast(null as CHAR(3)) PHONE_CODE, concat_ws('', PHONE_AREA_CODE, PHONE_NUMBER, PHONE_NUMBER_INTL) Phone_Num, 
			cast(null as CHAR(400)) Pref_Name, PHONE_AREA_CODE, PHONE_NUMBER, PHONE_NUMBER_INTL
    ...
    update `PECI_Phones` 
	 inner join cas.cc_gen_peci_phone_data_t phone
	    on Phone_Num = concat_ws('', phone.PHONE_AREA_CODE, phone.PHONE_NUMBER, phone.PHONE_NUMBER_INTL)
	...
	select PECI_PHONE_CODE, PHONE_CODE, Phone_Num, Pref_Name, PHONE_AREA_CODE, PHONE_NUMBER, PHONE_NUMBER_INTL from PECI_Phones;
